...
class CitySelector extends React.Component {
    ...
    componentDidMount() {
        const defaultCity = Object.keys(CITY_NODES)[0];
        this.props.onSelectCity(CITY_NODES[defaultCity]);
    }
    ...
    render() {
        return (
            <select onChange={this.onChange}>
                {
                    Object.keys(CITY_NODES).map(
                        cityName => <option key={cityName} value={CITY_NODES[cityName]}>{cityName}</option>
                    )
                }
            </select>
        );
